...
const adminService = {
  createUserAccount: (data) =>
    request("POST", "/v1/identities/register-customer", data),
  registerOtherAccounts: (data) =>
    request("POST", "/v1/identities/register-other-accounts", data, {
  ...
  updateProduct: (productId, data) =>
    request("PUT", `/v1/products/${productId}`, data, {
      "Content-Type": "multipart/form-data",
    }),
  deleteProduct: (productId) => request("DELETE", `/v1/products/${productId}`),

  viewProductsDetails: (productId) =>
    request("GET", `/v1/products/admin/${productId}`),
  createProductsDetails: (data) =>
    request("POST", "/v1/product-details", data, {
      "Content-Type": "multipart/form-data",
    }),
  updateProductsDetails: (productDetailId, data) =>
    request("PUT", `/v1/product-details/${productDetailId}`, data, {
      "Content-Type": "multipart/form-data",
    }),
  ...
  createCategory: (data) => request("POST", "/v1/categories", data),
  ...
  getAllWeights: (params) => request("GET", "/v1/weights", null, {}, params),
  createWeight: (data) => request("POST", "/v1/weights", data),
  updateWeight: (weightId, data) =>
    request("PUT", `/v1/weights/${weightId}`, data),
  deleteWeight: (weightId) => request("DELETE", `/v1/weights/${weightId}`),

  getAllFlavours: (params) => request("GET", "/v1/flavours", null, {}, params),
  createNewFlavour: (data) => request("POST", "/v1/flavours", data),
  updateFlavour: (flavourId, data) =>
    request("PUT", `/v1/flavours/${flavourId}`, data),
  deleteFlavour: (flavourId) => request("DELETE", `/v1/flavours/${flavourId}`),
};
